  ...
  <div class="box">
    <div class="list">
      <el-table :data="tableData" style="width: 100%" height="420">
        <el-table-column fixed prop="id" label="专题序号" width="150" />

        <el-table-column prop="title" label="专题名称" width="350" />
        <el-table-column label="专题图片" width="250">
          <template #default="scope">
            <img :src="scope.row.imgpath" alt="">
          </template>
        </el-table-column>

        <el-table-column label="专题设置" fixed="right" width="180">
          <template #default="scope">
...
onMounted(()=>{
  that.$http.getAll({typeId:'专题推荐'}).then(res=>{
    res.forEach(item=>{
