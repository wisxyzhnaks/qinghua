  ...
  <div class="box">
    <div class="list">
      <el-table :data="tableData" style="width: 100%" height="420">
        <el-table-column fixed prop="id" label="新闻序号" width="120" />

        <el-table-column prop="title" label="新闻名称" width="250" />
        <el-table-column prop="context" label="新闻介绍" width="600" />
        <el-table-column label="新闻图片" width="250">
          <template #default="scope">
            <img :src="scope.row.imgpath" alt="">
          </template>
        </el-table-column>

        <el-table-column label="新闻设置" fixed="right" width="150">
          <template #default="scope">
...
onMounted(()=>{
  that.$http.getAll({typeId:'头条新闻'}).then(res=>{
    res.forEach(item=>{
